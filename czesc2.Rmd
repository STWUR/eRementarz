...
mieszkania %>%
  group_by(dzielnica) %>%
  summarise(metraz = sum(metraz)) %>%
  inner_join(ludnosc, by = c("dzielnica"="Dzielnica")) %>%
  mutate(m2_na_mieszkanca = metraz/liczba_mieszkancow/1000)
```
...
```{r}
mieszkania %>%
  mutate(rodzaj_zabudowy = case_when(pietro_maks < 3 ~ "mniej niż 3 piętra",
                                     pietro_maks <= 6 ~ "3 do 6 pięter",
                                     TRUE ~ "ponad 6 pięter")) %>%
  select(pietro_maks, rodzaj_zabudowy)
```
